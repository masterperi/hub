...
export default function ManageLeaveWindowScreen() {
    ...
    const updateSettingsMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            Alert.alert("Success", "Leave window updated successfully!");
        },
        onError: (error) => {
            Alert.alert("Error", error.message || "Failed to update leave window");
        }
    });

    const handleSave = () => {
        if (!label.trim()) {
            Alert.alert("Error", "Please enter a label for this leave session (e.g., Summer Break)");
            return;
        }
        if (toDate < fromDate) {
            ...
            {!isLoading && (
                <ScrollView contentContainerStyle={styles.scrollContent}>
                    <View style={[styles.card, { backgroundColor: theme.backgroundDefault }]}>
                        <ThemedText type="h3" style={styles.title}>Configure Leave Window</ThemedText>
                        <ThemedText secondary style={styles.subtitle}>Set the official leave dates for {user?.hostelBlock}. This will be visible to all students in your hostel.</ThemedText>
                        <View style={styles.form}>
                            <View style={styles.inputGroup}>
                                <ThemedText type="bodySmall" secondary style={styles.label}>Leave Session Label</ThemedText>
                                <TextInput
                            ...
                            {settings?.leaveWindowLabel ? (
                                <Button
                                    ...
                                    fullWidth
                                >
                                    <ThemedText style={{ color: Colors.status.error }}>Clear Session Info</ThemedText>
                                </Button>
                            ) : null}
                        </View>
                    </View>
