...
export default function AdminDashboardScreen() {
  ...
  const CAROUSEL_ITEMS = [
    ...
    { id: 2, title: "Breakfast Review", subtitle: "Positive feedback today", bg: [Colors.status.success, "#059669"], icon: "coffee" },
  ...
  const adminStats = stats as any || { studentCount: 0, attendanceCount: 0, pendingLeaveCount: 0, openComplaintCount: 0, pendingRoomChanges: 0 };
    ...
    <ThemedView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.statsGrid}>
          <StatCard icon="users" title="Total Students" value={adminStats.studentCount} color={Colors.primary.main} />
          ...
          <StatCard icon="home" title="Room Requests" value={adminStats.pendingRoomChanges || 0} color={Colors.secondary.main} onPress={() => navigation.getParent()?.navigate("ApprovalsTab", { screen: "RoomChangeApprovals" })} showAlert={adminStats.pendingRoomChanges > 0} />
          <StatCard icon="navigation" title="Leave Session" value={activeSession?.leaveWindowLabel || "None"} color={Colors.primary.main} onPress={() => navigation.getParent()?.navigate("ManageTab", { screen: "ManageLeaveWindow" })} showAlert={!!activeSession?.leaveWindowLabel} />
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(300)} style={styles.actionsGrid}>
          ...
          <Pressable style={[styles.actionCard, { backgroundColor: theme.backgroundSecondary }]} onPress={() => navigation.getParent()?.navigate("ApprovalsTab", { screen: "RoomChangeApprovals" })}><View style={[styles.actionIcon, { backgroundColor: Colors.primary.main + '20' }]}><Feather name="refresh-cw" size={24} color={Colors.primary.main} /></View><ThemedText type="body" style={styles.actionText}>Room Changes</ThemedText></Pressable>
          <Pressable style={[styles.actionCard, { backgroundColor: theme.backgroundSecondary }]} onPress={() => navigation.getParent()?.navigate("ManageTab", { screen: "ManageLeaveWindow" })}><View style={[styles.actionIcon, { backgroundColor: Colors.status.warning + '20' }]}><Feather name="navigation" size={24} color={Colors.status.warning} /></View><ThemedText type="body" style={styles.actionText}>Leave Session</ThemedText></Pressable>
        </Animated.View>
      </ScrollView>
