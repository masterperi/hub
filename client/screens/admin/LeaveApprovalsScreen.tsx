...
import { StyleSheet, View, ScrollView, Pressable, Modal, TextInput, Alert, FlatList, Image, Animated as RNAnimated, Easing, Platform } from "react-native";
            ...
            {selectedRequest && (
              <ScrollView contentContainerStyle={styles.modalBody} showsVerticalScrollIndicator={false}>
                ...
                {selectedRequest.imageUrl && (
                  <View style={styles.attachmentSection}>
                    ...
                    <Pressable
                      ...
                      onPress={() => Alert.alert("Attachment Link", selectedRequest.imageUrl)}
                    >
                    ...
                    <Image source={{ uri: selectedRequest.imageUrl }} style={styles.fullImage} />
                  </View>
                )}
                ...
                {selectedRequest.status === "pending" ? (
                  ...
                  <View style={[styles.statusBanner, { backgroundColor: STATUS_COLORS[selectedRequest.status as keyof typeof STATUS_COLORS] + '20' }]}>
                    <ThemedText style={{ color: STATUS_COLORS[selectedRequest.status as keyof typeof STATUS_COLORS], fontWeight: 'bold', textAlign: 'center' }}>
                      Request {selectedRequest.status.charAt(0).toUpperCase() + selectedRequest.status.slice(1)}
                    </ThemedText>
                    {selectedRequest.adminRemarks && <ThemedText type="caption" style={{ textAlign: 'center', marginTop: 4 }}>"{selectedRequest.adminRemarks}"</ThemedText>}
                  </View>
                )}
              </ScrollView>
            )}
...
const styles = StyleSheet.create({
  ...
  fullImage: { width: "100%", height: 200, borderRadius: BorderRadius.md, resizeMode: 'cover', backgroundColor: '#f0f0f0' },
