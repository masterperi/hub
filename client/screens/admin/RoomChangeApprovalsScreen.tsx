...
export default function RoomChangeApprovalsScreen() {
    ...
    const { data: requests, isLoading } = useQuery({
        queryKey: ['/api/room-change-requests/hostel', user?.hostelBlock],
    ...
    const { data: targetRoom, isFetching: isCheckingRoom } = useQuery({
        queryKey: ["/api/rooms", selectedRequest?.requestedRoom, user?.hostelBlock],
    ...
    const updateStatusMutation = useMutation({
        mutationFn: async ({ id, status, remarks }: { id: string; status: string; remarks: string }) => {
            const res = await apiRequest("PUT", `/api/room-change-requests/${id}`, { status, adminRemarks: remarks });
            if (!res.ok) {
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/room-change-requests/hostel'] });
    ...
    return (
        <ThemedView style={styles.container}>
            ...
            <FlatList
                ...
                contentContainerStyle={{ padding: Spacing.lg, paddingBottom: tabBarHeight + 100 }}
                renderItem={({ item }) => (
                    <Pressable
